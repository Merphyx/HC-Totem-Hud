...
public class HudRenderer {
    ...
    public static void render(DrawContext drawContext) {
        ...
        if (CONFIG.warningToggle) {
            if (totemCharges == CONFIG.warningThreshold && !warningShown) {
                sendWarningMessage(client);
                warningShown = true;
            } else if (totemCharges > CONFIG.warningThreshold) {
                warningShown = false;
            }
        }

        if (CONFIG.soundToggle) {
            if (totemCharges == CONFIG.warningThreshold && !soundPlayed) {
    ...
    private static void sendWarningMessage(MinecraftClient client) {
        if (client.player != null) {
            client.player.sendMessage(Text.literal("§c§lWarnung §e⚠ §8» §fDu hast nur noch §e" + TotemCheck.countTotemCharges() + "x §fLeben!"), false);
        }
    }
